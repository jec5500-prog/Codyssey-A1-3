    ...
    <div className="space-y-8">
      ...
      {!loading && report && !report.isSufficient && (
        <div className="bg-zinc-900 border border-amber-500/50 rounded-3xl p-8 text-center space-y-6 shadow-2xl">
          ...
          <div className="pt-2">
            <Link
              href="/capture"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-indigo-600 text-white font-bold text-xs shadow-lg shadow-cyan-500/30 hover:scale-105 transition-transform"
            >
      ...
      {!loading && report && report.isSufficient && (
        <div className="space-y-8">
          ...
          <div className="bg-gradient-to-br from-zinc-900 via-zinc-900 to-cyan-950/50 border border-zinc-800 p-6 sm:p-8 rounded-3xl space-y-4">
                ...
                {report.materialTrends.map((mat, idx) => (
                  <div key={idx} className="bg-zinc-950 p-3.5 rounded-2xl border border-zinc-800/80 space-y-2">
                    <div className="flex items-center justify-between text-xs font-bold text-zinc-200">
                      ...
                      <span className="font-mono text-cyan-400">{mat.percentage}% Share</span>
                    </div>
                    <div className="w-full h-2 bg-zinc-900 rounded-full overflow-hidden border border-zinc-800">
                      <div className="h-full bg-gradient-to-r from-cyan-500 to-indigo-500 rounded-full" style={{ width: `${mat.percentage}%` }} />
                    </div>
                    <p className="text-[11px] text-zinc-400 italic">{mat.insight}</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2">
                {report.styleEvolution.map((st, i) => (
                  <div key={i} className="bg-zinc-950 p-3 rounded-xl border border-zinc-800/80 text-xs">
                    ...
                    <span className="text-[11px] text-zinc-400 mt-0.5 block">{st.shift}</span>
                  </div>
                ))}
              </div>
            </div>
